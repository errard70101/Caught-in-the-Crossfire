...
\section*{III. Contents of Grant Proposal}

\section{Research Project's Background}

\subsection{Economic Uncertainty in Small Open Economies}
...
\subsubsection{Economic Uncertainty: Measurement and Effects}
...
\subsubsection{Uncertainty in Small Open Economies}
...
For Taiwan specifically, existing studies have explored various uncertainty dimensions. However, these studies have not employed the methodologically rigorous, large-scale, order-invariant framework that DHK demonstrate is necessary for credible inference.

\subsubsection{SVMVAR Models and Order Invariance}
...
\subsubsection{US-China Relations and Taiwan's Economy}
...
\subsubsection{(1) Research Principles, Methods, and the Innovation of Research Methods}

\paragraph{The Order-Invariant SVMVAR Framework}
...
\subsubsection{(2) Anticipated Problems and Means of Resolution}
...
\subsubsection{(2.1) Works Planned to be Done in the First Year}

The first year will focus on data collection, model implementation, and preliminary estimation:

\paragraph{Months 1--3: Data Collection and Preparation}
\begin{itemize}
...
\paragraph{Detailed Data Requirements}

Following DHK's emphasis on model size, we aim to construct a dataset with 43+ monthly frequency variables spanning approximately 1990--present (or the longest available period).

\textbf{Macroeconomic Variables (19 variables)}

\textit{Output and Activity:}
\begin{enumerate}
...
\textbf{Financial Variables (9 variables)}

\textit{Interest Rates and Spreads:}
\begin{enumerate}
...
\textbf{Unclassified Variables (16+ variables)}

\textit{A. External Shock Sources (10+ variables)} --- All external variables are unclassified
...
\textit{Global Risk and US-China Relations:}
\begin{enumerate}
    \setcounter{enumi}{7}
...
\textit{B. Taiwan Domestic Ambiguous Variables (6+ variables)}
...
\paragraph{Months 4--6: Model Implementation}
\begin{itemize}
...
By the end of the first year, we expect to achieve the following:

\begin{itemize}
    \item \textbf{Complete Dataset}: A comprehensive database of 43+ monthly variables covering Taiwan macroeconomic, financial, and external indicators with proper documentation
    \item \textbf{Validated Model Implementation}: A fully functional OI-SVMVAR estimation code validated against DHK's published results
    \item \textbf{Preliminary Estimates}: Initial estimation results from small-scale models (30 variables) to assess data quality and model performance
    \item \textbf{Methodological Foundation}: Established computational infrastructure and validated estimation procedures ready for full-scale analysis
    \item \textbf{Conference Presentation}: Present preliminary findings at domestic economics conference
\end{itemize}
...
The second year focuses on implementing the three-step analytical framework:

\paragraph{Step 1: Time-Varying Classification Analysis}

\textbf{Method}: Analyze time-varying classification probabilities $\pi_{i,t}$ for external variables

\textbf{Output}: Time-series plots showing when external variables (VIX, US FFR, China IPI, US-China relations indices) are classified as ``macroeconomic'' ($\pi_{i,t} \to 1$) versus ``financial'' ($\pi_{i,t} \to 0$)

\textbf{Research Question}: When did US monetary policy affect Taiwan through financial channels versus macroeconomic channels? How did this shift during major episodes (2008 crisis, 2018 trade war, 2020 pandemic)?

\paragraph{Step 2: Forecast Error Variance Decomposition (FEVD)}

\textbf{Method}: Decompose variance of Taiwan's latent uncertainty factors into contributions from each external source

\textbf{Output}: Percentage of Taiwan's $h_{m,t}$ (macro uncertainty) and $h_{f,t}$ (financial uncertainty) explained by:
\begin{itemize}
    \item US macroeconomic shocks (FFR, IPI)
    \item US financial shocks (credit spreads, EPU)
    \item China shocks (IPI, PPI, credit conditions)
    \item Global risk shocks (VIX, GPR)
    \item US-China relations shocks (trade policy uncertainty)
\end{itemize}

\textbf{Research Question}: Which external shock source contributes most to Taiwan's uncertainty? Is Taiwan more vulnerable to US shocks, China shocks, or bilateral relationship deterioration?

\paragraph{Step 3: Impulse Response Functions (IRF)}

\textbf{Method}: Trace dynamic responses of Taiwan variables to identified uncertainty shocks

\textbf{Output}: IRF charts showing how Taiwan's GDP, employment, TAIEX, exchange rate respond to:
\begin{itemize}
    \item One-standard-deviation shock to $h_{m,t}$ (Taiwan macro uncertainty)
    \item One-standard-deviation shock to $h_{f,t}$ (Taiwan financial uncertainty)
    \item Identified external shocks from Step 2
\end{itemize}

\textbf{Research Question}: What are the real economic impacts of uncertainty shocks transmitted through different channels?
...
\textbf{Challenge}: Adapting DHK's framework to Taiwan context may require modifications.
...
\begin{itemize}
    ...
    \item Consider simpler extensions if full adaptation proves infeasible
\end{itemize}

\paragraph{Interpretation and Policy Relevance}
...
\subsubsection{(2.1) Works Planned to be Done in the Second Year}

\paragraph{Months 7--10: Estimation and Analysis}
\begin{itemize}
...
\paragraph{Months 11--14: Interpretation and Policy Analysis}
\begin{itemize}
    \item Interpret findings in Taiwan's economic and institutional context
    \item Compare results across different historical episodes (1997, 2008, 2015, 2018, 2020, 2022)
    \item Develop policy implications for CBC and other stakeholders
    \item Conduct counterfactual analyses
\end{itemize}

\paragraph{Months 15--18: Writing and Dissemination}
\begin{itemize}
...
By the end of the second year, we expect to achieve the following:
...
This research will provide the first rigorous, model-based evidence on \textit{which channels} (macroeconomic vs. financial) external shocks use to impact Taiwan.
...
Using FEVD, we will answer: ``Is Taiwan more vulnerable to US shocks, China shocks, or bilateral relationship deterioration?''

\textbf{4. Detection of model size bias in Taiwan uncertainty research}

By comparing small-model versus large-model estimates, we will demonstrate the necessity of large-scale models for credible small open economy research.

\subsubsection{Policy Contributions}
...
\subsubsection{Expected Publications and Dissemination}

\begin{itemize}
    \item \textbf{Journal Submission}: Target top-tier field journals (Journal of International Economics, Journal of Applied Econometrics, Review of Economics and Statistics)
    \item \textbf{Working Paper}: NBER or similar working paper series
    \item \textbf{Policy Brief}: Accessible policy brief for Central Bank of China (Taiwan) and government agencies
    \item \textbf{Conferences}: Present at international conferences (American Economic Association, Econometric Society, Asian Meeting of Econometric Society)
\end{itemize}
...
This two-year research project represents an ambitious but feasible study to investigate economic uncertainty in Taiwan using state-of-the-art econometric methods. The integration of the two years' work proceeds as follows:

\subsection{Methodological Continuity}

The first year establishes the methodological foundation by implementing DHK (2025)'s order-invariant SVMVAR framework and adapting it to Taiwan's context. This includes:
\begin{itemize}
    \item Comprehensive data collection covering Taiwan domestic and external variables
    \item Validated MCMC estimation algorithm
    \item Preliminary model estimates to ensure computational feasibility
\end{itemize}

The second year builds on this foundation to conduct the three-step analytical framework:
\begin{itemize}
    \item Step 1: Time-varying classification analysis to identify transmission channels
    \item Step 2: FEVD to quantify relative importance of external shock sources
    \item Step 3: IRF analysis to trace economic consequences
\end{itemize}

\subsection{Research Coherence}

By applying the methodologically rigorous, order-invariant SVMVAR framework of Davidson, Hou, and Koop (2025) to Taiwan's unique economic context, this integrated research will:
...
This research has implications beyond Taiwan:

\begin{itemize}
    \item \textbf{Methodological}: Demonstrates how DHK's framework can be applied to answer fundamentally different questions than originally intended
    \item \textbf{Regional}: Provides generalizable framework for other small open economies in Asia-Pacific region facing similar dual exposures
    \item \textbf{Policy}: Offers concrete guidance for central banks in small open economies on designing state-contingent policy responses to external shocks
\end{itemize}

The project's success will depend on close coordination between the methodological work in Year 1 and the analytical implementation in Year 2, with continuous feedback loops to ensure policy relevance throughout the research process.
...
Sin, C.-Y. (2015).
\newblock The Economic Fundamental and Economic Policy Uncertainty of Mainland China and Their Impacts on Taiwan and Hong Kong.
\newblock {\em North American Journal of Economics and Finance}, 40, 298-311.
