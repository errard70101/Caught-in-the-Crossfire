...
def run_case(*, n: int, T: int, p: int, lam: float, sv_sigma: float, seed: int, n_vecs: int = 20) -> float:
    dgp = sample_dgp(n=n, T=T, p=p, seed=seed, sv_sigma=sv_sigma, lam=lam)
...
def main() -> int:
    cases = [
        dict(n=3, T=20, p=1, lam=1e2, sv_sigma=0.1),
        dict(n=3, T=20, p=2, lam=1e4, sv_sigma=0.3),
        dict(n=5, T=30, p=2, lam=1e4, sv_sigma=0.5),
        dict(n=5, T=60, p=3, lam=1e6, sv_sigma=0.8),
        dict(n=7, T=45, p=4, lam=1e4, sv_sigma=0.3),
    ]
    seeds = [0, 1, 7, 42]

    fail = False
    print(f"{'case':<48} {'seed':>4} {'max_rel_err':>14}")
    print("-" * 70)
    for c in cases:
        for s in seeds:
            worst = run_case(seed=s, **c)
            status = "OK" if worst < TOL else "FAIL"
            tag = f"n={c['n']} T={c['T']} p={c['p']} lam={c['lam']:.0e} sv={c['sv_sigma']}"
            print(f"{tag:<48} {s:>4} {worst:>14.3e}  {status}")
            if worst >= TOL:
                fail = True
