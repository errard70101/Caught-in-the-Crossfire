...
Taiwan provides a particularly important setting for studying external uncertainty transmission. It is deeply connected to the United States through financial markets, monetary conditions, and technology demand, and it is closely linked to China through trade, production networks, and cross-strait risk. At the same time, Taiwan occupies a central position in global semiconductor supply chains. These features make Taiwan highly exposed to external uncertainty. As Figure~\ref{fig:taiwan-vix-events} shows, Taiwan's option-implied volatility rises sharply around several major global and regional uncertainty episodes.

\begin{figure}[ht]
    \centering
    \caption{Taiwan VIX and major uncertainty episodes}
    \includegraphics[width=0.98\textwidth]{../../../figures/tw_taifex_vix_bloom_style.pdf}
    \label{fig:taiwan-vix-events}
\end{figure}

Exposure to these external sources makes identification difficult because the same external disturbance can reach Taiwan through several channels at once. A U.S. monetary tightening, for example, may affect Taiwan through capital flows and exchange rates, but it may also affect Taiwan through export demand and firms' production expectations. Similarly, a China slowdown may operate through trade and supply-chain linkages in one episode and through financial-market sentiment in another. The same external shock may therefore look financial in one episode, macroeconomic in another, and mixed in periods of severe stress.

This project therefore asks how external uncertainty shocks from the United States, China, and the global economy operate in Taiwan, and which structural friction explains their macroeconomic effects. It answers this question by moving from \emph{source}, to \emph{channel}, to \emph{mechanism}. Year 1 identifies the source and reduced-form transmission channel by estimating Taiwan's latent uncertainty factors and classifying external drivers based on their time-varying association with these factors. However, reduced-form evidence alone cannot distinguish between competing economic theories. For instance, a financial-channel response could be driven by intertemporal collateral constraints or static working-capital frictions.

Year 2 addresses this challenge by providing a \emph{structural mapping}: it derives a projection framework that converts second-moment uncertainty risk corrections into first-moment Business Cycle Accounting (BCA) wedges. This theoretical bridge allows us to define unique ``diagnostic signatures'' for each friction---predicting, for example, that collateral shocks should load primarily on the investment wedge while working-capital frictions should load on the after-tax labor wedge. Finally, Year 3 validates these mechanisms empirically by constructing Taiwan's BCA wedges and computing MCMC-integrated wedge impulse response profiles to test which theoretical signature best matches the empirical response. By integrating these three stages, the project moves beyond simple reduced-form descriptions to provide a structural diagnosis of Taiwan's external vulnerabilities.

\subsection{Open Challenges and Relation to the Literature}

A direct identification of how external uncertainty reaches Taiwan must overcome four overlapping difficulties. Each has been engaged separately by prior literature, but no existing study addresses them jointly. The four challenges below organize what is missing and motivate the methodological choices described in the next subsection.

The first challenge is that external and domestic sources of uncertainty are tightly intertwined. A single geopolitical or policy event---an FOMC tightening, a U.S.--China tariff round, or a cross-strait incident---may simultaneously alter global risk sentiment, Taiwan's domestic political outlook, and expectations about trade, production, and finance. In a small model with limited domestic controls, these domestic movements may be misattributed to external shocks, producing severe omitted-variable bias \citep{carriero2018measuring}. The macro-uncertainty literature has documented the broad effects of uncertainty on investment, hiring, and economic activity \citep{bloom2009impact,bloom2014fluctuations,baker2016measuring,handley2022tpu}, but these studies typically rely on a single proxy index or a small model and cannot separate domestic from external contributions. The closest Taiwan-focused precedent, \citet{huang2021taiwan}, constructs a Baker--Davis-style text-based EPU index for Taiwan and uses a Diebold--Yilmaz spillover model to examine linkages with U.S. and Japanese EPU indices, finding that roughly half of Taiwan's economic uncertainty originates domestically and that U.S. and Japanese spillovers are significant. Even this work, however, delivers a single scalar EPU index and a reduced-form spillover decomposition: it does not separate Taiwan's domestic uncertainty into macroeconomic and financial components, does not identify whether external uncertainty enters Taiwan through macroeconomic or financial channels, and does not estimate uncertainty jointly with its macroeconomic effects in one model. To close all three of these gaps, the present project builds a unified monthly dataset that combines Taiwan domestic anchors with U.S., Chinese, and global drivers, and estimates two latent uncertainty factors jointly with the VAR dynamics that govern their macroeconomic effects.

The second challenge is that the large information set needed to reduce omitted-variable bias creates its own identification problem. In high-dimensional VAR systems, standard Cholesky identification becomes fragile because the results depend on arbitrary variable orderings. At the same time, the strict block-exogeneity restrictions traditionally imposed in small-open-economy models are too rigid for the present question, because they predetermine how external and domestic variables can interact---a particularly costly assumption when the goal is to learn the operative transmission channel from the data. The order-invariant stochastic volatility in mean vector autoregression (OI-SVMVAR) framework of \citet{davidson2025investigating} resolves the ordering problem and accommodates large information sets without imposing recursive zero restrictions. Its ``unclassified-block'' mechanism has so far been used to classify ambiguous domestic variables, for example financial conditions indices that might plausibly belong to either the macroeconomic or financial block. It has not yet been adapted to identify the transmission channel of external shocks into a small open economy. The present project repurposes the unclassified block in exactly that way: external drivers are placed in the unclassified block, and the posterior classification probabilities then reveal whether each external driver co-moves with Taiwan's macroeconomic or financial uncertainty factor in any given month.

The third challenge concerns how uncertainty is measured and how the estimate is used downstream. Many existing approaches rely either on proxy indicators (such as the VIX or a text-based EPU index) or on a two-step design that first estimates uncertainty and then studies its macroeconomic effects in a separate model. Proxy indicators may not capture how uncertainty is experienced inside Taiwan, and two-step procedures treat the first-stage estimate as observed data, discarding estimation uncertainty and inducing measurement-error and model-consistency bias \citep{carriero2018measuring}. \citet{davidson2025investigating} addresses the first-stage problem by estimating uncertainty and its effects jointly. The literature has not, however, propagated first-stage posterior uncertainty into downstream structural-mechanism tests; downstream validation typically uses a point estimate of the latent uncertainty factor and a single posterior-mean impulse response. The present project closes this residual gap with an \textit{MCMC-integrated} wedge impulse response construction: each Year~1 OI-SVMVAR posterior draw delivers its own impulse response path, which is converted into a BCA wedge response through the accounting identities. The resulting credible bands integrate first-stage posterior uncertainty directly into the mechanism test, providing a coherent end-to-end Bayesian inference rather than a hybrid Bayesian-frequentist pipeline.

The fourth challenge is that even a well-identified reduced-form impulse response does not by itself reveal the underlying mechanism. A financial-channel response may reflect a collateral constraint, a time-varying risk premium, a working-capital friction, or several of these at once. The Business Cycle Accounting (BCA) literature \citep{chari2007business} provides a useful diagnostic by decomposing observed aggregate fluctuations into wedges that summarize distortions in prototype equilibrium conditions, and small-open-economy extensions add efficiency and external wedges that capture imported-input dependence and exchange-rate movements. BCA has, however, been used primarily as a positive accounting device for ordinary business-cycle dynamics; it has not been connected to uncertainty-driven second-moment risk corrections, and there is no existing wedge-projection framework that converts uncertainty-shock impulse responses into testable wedge signatures. The present project derives such a framework: it shows how collateral and risk-premium frictions should project primarily into the investment wedge, while working-capital frictions should project primarily into the after-tax labor wedge. The reduced-form transmission-channel evidence from Year~1 can then be matched against these structural signatures in Year~3, allowing the project to distinguish competing mechanisms rather than stopping at reduced-form descriptions.

Together these four gaps define the project's methodological contribution. The next subsection describes the empirical design that addresses them, and the final subsection explains how the design is implemented across the three years.

\subsection{Research Design and Main Contributions}

The empirical design responds to the four challenges of the previous subsection through three design choices, and yields one project-level methodological contribution plus three year-specific contributions.

\textbf{Joint estimation in a large information set.} The project uses the order-invariant stochastic volatility in mean vector autoregression (OI-SVMVAR) framework of \citet{davidson2025investigating} to estimate Taiwan's macroeconomic and financial uncertainty factors jointly with their VAR dynamics. The model allows both latent uncertainty factors to enter the conditional mean of the VAR, so uncertainty directly affects macroeconomic and financial outcomes rather than merely altering their variances. Joint estimation addresses the omitted-variable problem (C1) by accommodating a rich information set covering both domestic anchors and multiple external drivers; the order-invariant identification scheme addresses the large-VAR ordering problem (C2) without requiring restrictive block-exogeneity assumptions; and the single-stage Bayesian estimation removes the measurement-error bias of two-step procedures (C3).

\textbf{External drivers as an unclassified block.} The central adaptation is to repurpose the OI-SVMVAR unclassified-variable mechanism for transmission-channel identification. Taiwan's clearly real-side variables (industrial production, exports, employment, prices) anchor the domestic macroeconomic uncertainty factor, while Taiwan's clearly financial variables (equity returns, exchange-rate volatility, credit conditions) anchor the domestic financial uncertainty factor. External drivers---U.S. monetary stance, Chinese real-side cyclical indicators, global financial-market volatility, geopolitical risk, and trade-policy uncertainty---are placed in the unclassified block. The posterior classification probabilities then deliver a data-driven, time-varying measure of which channel each external driver activates in any given month, without requiring the researcher to assume the channel ex ante.

\textbf{Wedge projection and MCMC-integrated validation.} Reduced-form classification cannot by itself distinguish competing structural frictions (C4). Year~2 derives a wedge-projection framework that maps uncertainty-related second-moment risk corrections in a nonlinear structural model into Business Cycle Accounting wedges, converting transmission-channel evidence into testable structural predictions: collateral and risk-premium mechanisms should load primarily on the investment wedge, while working-capital mechanisms should load primarily on the after-tax labor wedge. Year~3 then constructs MCMC-integrated wedge impulse response profiles by propagating each OI-SVMVAR posterior draw through the BCA accounting identities, and compares these empirical profiles against the theoretical wedge generalized impulse response functions implied by the candidate mechanisms. By propagating first-stage posterior uncertainty into the structural test rather than relying on a point estimate, this MCMC-integrated design closes the residual two-step bias of C3 and yields a coherent end-to-end inference path.

The project's central \textbf{methodological contribution} is to repurpose the OI-SVMVAR unclassified-block mechanism. Where the original framework uses it to classify ambiguous domestic variables, the present project uses it to identify the transmission channel of external shocks into a small open economy. This reframing turns a domestic-variable classification tool into a structural diagnostic for external-shock transmission, and it makes the channel of transmission a posterior object rather than an ex ante assumption.

The project also produces three year-specific contributions. The \textbf{first-year contribution} is \emph{empirical identification}: the project constructs a monthly Taiwan external-uncertainty dataset, estimates Taiwan-specific macroeconomic and financial uncertainty factors, and computes time-varying classification probabilities for each external driver. The \textbf{second-year contribution} is \emph{theoretical mapping}: the project derives wedge signatures for collateral, risk-premium, and working-capital frictions in a small-open-economy environment, clarifying how each friction should appear in the investment, labor, efficiency, and external wedges. The \textbf{third-year contribution} is \emph{empirical validation}: the project constructs Taiwan's BCA wedges and uses the MCMC-integrated wedge impulse response profiles to test which structural signature best matches the reduced-form transmission patterns identified in Year~1.

\subsection{Three-Year Roadmap}

The first year produces the empirical foundation. Its main deliverables are (a) a monthly Taiwan external-uncertainty dataset that combines domestic macroeconomic anchors, domestic financial anchors, and external drivers from the United States, China, and the global economy, with variable lists, time spans, and sources documented in the first-year methodology section; (b) the joint posterior distribution of Taiwan's macroeconomic and financial uncertainty factors from the OI-SVMVAR estimation; (c) time-varying classification probabilities for each external driver, indicating whether it co-moves with Taiwan's macroeconomic or financial uncertainty in any given month; and (d) impulse responses and forecast error variance decompositions characterizing how each external uncertainty shock reaches Taiwan's real and financial variables.

The second year produces the theoretical mapping. Its main deliverables are (a) the mathematical derivation of how second-moment risk corrections in a nonlinear structural model project into Business Cycle Accounting wedges; (b) wedge signatures for the three competing frictions---collateral constraints, time-varying risk premia, and working-capital frictions---identifying which BCA wedge each friction loads on; and (c) a characterization of the efficiency and external wedges that arise specifically in a small open economy with imported-input dependence and exchange-rate exposure. The theoretical derivation is intentionally independent of Year~1's empirical outputs, so the second-year work proceeds in parallel with the first-year estimation rather than waiting for it to complete.

The third year produces the integrated empirical test. Its main deliverables are (a) Taiwan's BCA wedges constructed from national accounts data; (b) MCMC-integrated wedge impulse response profiles, built by propagating each Year~1 OI-SVMVAR posterior draw through the BCA accounting identities and benchmarked against the theoretical wedge generalized impulse responses from Year~2; (c) a comparative mechanism indicator---an RMSE-based fit criterion with correlation-based robustness checks---ranking the competing frictions against Taiwan's data; and (d) policy interpretation that connects the winning mechanism to specific instruments, such as asset-price and credit-market policy for collateral and risk-premium channels and short-term liquidity support for working-capital channels. The third-year estimation therefore depends on Year~1's posterior draws and on Year~2's wedge-signature hypotheses, and the project schedule places it only after both inputs are available.

Taken together, the three years yield a three-dimensional account of external uncertainty transmission to Taiwan: which source contributes most, which channel carries it, and which structural friction explains its macroeconomic effects. The integrated final deliverable is a source-by-channel-by-mechanism mapping in which each external driver is associated with its dominant transmission channel and its preferred structural interpretation, providing both an empirical diagnostic for monitoring purposes and a model-class prescription for forecasting under uncertainty.

\section{Preliminary Progress and Feasibility}

\subsection{Preliminary Progress}

The project is feasible because it builds on substantial preliminary work. For the first-year analysis, I have developed a preliminary raw-data collection pipeline for most variables in the proposed OI-SVMVAR design, and the dataset has moved from a purely conceptual plan to an operational working structure. The baseline monthly panel covers 2003M1--2025M12 because the preferred TAIEX total-return index begins in 2003. A separate robustness specification can extend the sample to 2001M1 by replacing total-return-based equity returns and volatility with measures constructed from the longer TAIEX price index. At this stage, the candidate baseline contains 52 variables, including 22 Taiwan macroeconomic variables, 8 Taiwan financial variables, and 22 unclassified variables that include external drivers and Taiwan-domestic variables whose transmission channel may vary over time. Ongoing work focuses on finalizing variable definitions, checking cross-source consistency, refining transformation rules, and documenting the reproducibility pipeline.
...
peak of a long-horizon impulse response. The next revision will compare the full horizon profile,
...
Beyond the standard transformations, I address special data-construction choices directly. For
example, Taiwan's
...
in generalized impulse responses. Third-order terms introduce additional
...
at a long horizon. A peak impulse response may occur many months after the
...
therefore be time-aggregated to quarterly frequency before entering the wedge-level GIRF computations. The baseline aggregation will use quarterly averages of the monthly posterior draws,
...
\addlinespace
Wedge impulse response profiles can be sensitive to horizon choice and identification assumptions. &
Report full horizon profiles, including impact responses, peak responses, and sign changes, and
compare against the theoretical generalized impulse response across the full horizon rather than
at a single long-horizon peak. \\
\addlinespace
First-stage uncertainty factors create generated-regressor concerns. &
Propagate posterior uncertainty from the first-year OI-SVMVAR directly into the third-year wedge
impulse response construction, with each posterior draw producing its own BCA-wedge response
path, instead of conditioning on posterior-mean factors. \\
\bottomrule
\end{tabular}
\end{center}
