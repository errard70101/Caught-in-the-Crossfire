% batch_compute_wedges_full.m
clear; clc;
conf = get_project_config();
addpath(fullfile(conf.LOCAL_REPO_ROOT, 'code', 'empirical_bca_us', 'matlab', 'utils'));

% 1. Load the original SVAR results (full sample)
res_file = fullfile(conf.CROWN_SVAR_DIR, 'est_result.mat');
if ~isfile(res_file)
    error('Full sample estimation result not found!');
end
load(res_file);
...
for s_idx = 1:2
    ...
    temp_script = fullfile(conf.TEMP_DIR, ['temp_svar_run_full_' current_name '.m']);
    ...
    % Aggregation: 68% Credible Interval (16th/84th quantiles)
    wedges = struct();
    ...
    save(fullfile(conf.OUTPUT_DIR, sprintf('empirical_wedges_ci_us_%s.mat', current_name)), 'wedges');
end
exit;
