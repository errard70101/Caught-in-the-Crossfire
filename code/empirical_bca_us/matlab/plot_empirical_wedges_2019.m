% plot_empirical_wedges_2019.m
clear; clc;
conf = get_project_config();

for shock_type = {'macro', 'fin'}
    ts = shock_type{1};
    data_file = fullfile(conf.OUTPUT_DIR, sprintf('empirical_wedges_ci_us_%s_2019.mat', ts));
    
    if ~isfile(data_file)
        error('Data file not found: %s. Run batch_compute_wedges_2019.m first.', data_file);
    end
    load(data_file);
    ...
    title_str = sprintf('U.S. Empirical Wedge IRFs to %s Shock\n(Pre-COVID Sample: 1959-2019, 68%% Credible Interval, Quarterly Data)', type_label);
    title(t, title_str, 'FontWeight', 'bold', 'FontSize', 14);

    out_fig = fullfile(conf.OUTPUT_DIR, sprintf('us_empirical_wedges_ci_%s_2019.png', ts));
    saveas(fig, out_fig);
end
exit;
